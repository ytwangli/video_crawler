...
def get_oplog(site):
    ...
    cur.execute("select id, site, id_start, id_end, num, num_long from oplog where status=1 and site=%s order by id asc LIMIT 32;",(site,))
    for data in cur.fetchall():
...
def update_oplog(id):
    ...
    cur.execute('update oplog set status=2 where id=%s;',(id,))
    conn.commit()
    close_conn(conn, cur)
...
def update_oplog_list(list):
    conn, cur = open_conn_w()
    for id in list:
        cur.execute('update oplog set status=2 where id=%s;',(id[0],))
    conn.commit()
    close_conn(conn, cur)
...
def send_check_data(send_data):
    ...
    if len(report_list) > 0:
        send_report = '\r\n'.join(report_list)
        if send_mail(mailto_list,u"站外版权监控最新结果",send_report):
            print "send mail ok !"
        else:
...
if __name__ == '__main__':
    ...
    '''
    site_dict = {'acfun':'http://www.acfun.tv/v/ac'
                ,'bilibili':'http://www.bilibili.com/video/av'
                }
    ...
    keyword_list = [u'结成友奈是勇者',u'一休与淘气公主',u'美食神的超食宝',u'圣域传说',
                    u'JOJO的奇妙冒险 第1季',u'航海王#1-709',u'龙珠',u'圣斗士星矢',
                    u'灌篮高手',u'新大头儿子小头爸爸 第三季',u'数码宝贝大冒险 TRI',u'航海王TVSP Adventure of Nebulandia',
                    u'Go！Princess 光之美少女#49-50',u'境界触发者#51-74',u'百变马丁',u'爆笑虫子 第3季',
                    u'赛尔号6',u'圣斗士星矢黄金魂']
