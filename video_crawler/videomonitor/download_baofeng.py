...
def download_video(para):
    ...
    try:
        ...
        if not os.path.exists(jpg_path1):
            video_url_cdn = Baofeng().parse(idstr)
            if video_url_cdn[0] == '-':
                status = video_url_cdn
                Bad_Datas.append([idstr, '', status])
                return None
            size = HTMLResource(video_url_cdn, headers=headers).download_video(video_path, 1024*1024*10)
            if size is not None and size > 0:
                ...
                if os.path.exists(jpg_path):
                    ...
                    size = HTMLResource(video_url_cdn, headers=headers).download_video(video_path, 1024*1024*25)
                    if size is not None and size > 0:
                        returncode = Util.movie_split_image(video_path, jpgdir, idstr)
                        if os.path.exists(jpg_path):
                            DataBase.update_data(site, idstr, date_str)
...
def download_by_day():
    ...
    runday = datetime.date(2015,6,1)
    days = (today - runday).days + 1
    print runday
    data_list, bad_data_list = DataBase.get_datas(site, 'status = 0 and info_site LIKE "%bf-%"')
