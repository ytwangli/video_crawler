...
image_base_path = "/media/2mage/"
video_base_path = "/media/2mage/video/"
site = 'tudou'
Bad_Datas = []
headers = Tudou().get_headers()
...
def download_video(para):
    ...
    try:
        ...
        if not os.path.exists(jpg_path1):
            video_url_cdn = Tudou().parse(idstr)
...
def download_by_day_hour():
    ...
    while(i < count):
        ...
        if i % 500 == 0 or i >= count:
            ...
            Util.delete_empty_dir(jpgdir)
            if os.path.exists(jpgdir):
                if len(os.listdir(jpgdir)) > 300 or i >= count:
                    DataBase.insert_pathdata(site, image_base_path, date_str)
                    hour += 1
            if hour >= 20:
                hour = 10
