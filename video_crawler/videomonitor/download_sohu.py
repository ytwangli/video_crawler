...
def download_video(para):
    ...
    try:
        ...
        if not os.path.exists(jpg_path1):
            video_m3u8_urls = Sohu().parse(idstr)
            if len(video_m3u8_urls[0]) < 10:
                code = video_m3u8_urls
                Bad_Datas.append([idstr, '', code])
                return None
            size = HTMLResource('', headers=headers).download_video_ts(video_m3u8_urls, video_path, 1024*1024*20)
            if size is not None and size > 0:
                ...
                if os.path.exists(jpg_path):
                    ...
                    size = HTMLResource('', headers=headers).download_video_ts(video_m3u8_urls, video_path, 1024*1024*45)
                    if size is not None and size > 0:
                        returncode = Util.movie_split_image(video_path, jpgdir, idstr)
                        if os.path.exists(jpg_path):
                            DataBase.update_data(site, idstr, date_str)
...
def download_by_day():
    ...
    runday = datetime.date(2015,12,15)
