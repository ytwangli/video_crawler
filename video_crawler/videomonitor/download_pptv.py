...
def download_video(para):
    ...
    try:
        ...
        if not os.path.exists(jpg_path1):
            video_url = PPTV().parse(idstr)
            if video_url == '-1':
                Bad_Datas.append([idstr, '', video_url])
                return None
            size = HTMLResource(video_url, headers=headers).download_video(video_path, 1024*1024*10)
            if size is not None and size > 0:
                ...
                if os.path.exists(jpg_path):
                    ...
                    size = HTMLResource(video_url, headers=headers).download_video(video_path, 1024*1024*20)
                    if size is not None and size > 0:
                        returncode = Util.movie_split_image(video_path, jpgdir, idstr)
                        if os.path.exists(jpg_path):
                            DataBase.update_data(site, idstr, date_str)
...
def download_by_day():
    ...
    runday = datetime.date(2015,11,28)
