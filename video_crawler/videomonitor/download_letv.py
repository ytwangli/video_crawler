...
def download_video(para):
    ...
    try:
        ...
        if not os.path.exists(jpg_path1):
            video_m3u8_urls = Letv().parse(idstr)
            if video_m3u8_urls[0] == '-':
                status = video_m3u8_urls
                Bad_Datas.append([idstr, '', status])
                return None
            size = HTMLResource('', headers=headers).download_video_ts(video_m3u8_urls, video_path, 1024*1024*100)
            if size is not None and size > 0:
                ...
                if os.path.exists(jpg_path):
                    ...
                    size = HTMLResource('', headers=headers).download_video_ts(video_m3u8_urls, video_path, 1024*1024*200)
                    if size is not None and size > 0:
                        returncode = Util.movie_split_image(video_path, jpgdir, idstr)
                        if os.path.exists(jpg_path):
                            DataBase.update_data(site, idstr, date_str)
