...
def download_video(para):
    ...
    try:
        ...
        jpgdir1 = image_base_path+'acfuntmp'+'/' + cidstr + '/'
        if os.path.exists(jpgdir1):
            os.renames(jpgdir1, jpgdir)
        jpg_path = jpgdir + cidstr + '_20.jpg'
        jpg_path1 = jpgdir + cidstr + '_5.jpg'
        if not os.path.exists(jpg_path1):
            video_url_cdn = Acfun().parse(str(cid))
            if video_url_cdn[0] == '-':
...
def download_by_day():
    ...
    print runday
    data_list, bad_data_list = DataBase.get_ciddatas(site, 'status=0')
