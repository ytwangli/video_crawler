...
//--------------------------mysql-----------------------
#define HOST_R 				"10.121.86.156"
#define HOST_W 				"10.121.86.156"
#define USER 				"root"
#define PASSWD 				"iloveiqiyi"
#define DB 					"mysql"
#define PORT 				3306
//#define PORT 				8597
//------------------------------
...
void* thread_getImageList(void *arg)
{
	...
	//sprintf(szSql, "SELECT site,image_dir,key_type,key_path,key_detail,new_flag FROM %s WHERE host=%d AND code_flag=0 AND used_flag=1;", (char*)g_strTable.c_str(),ihost );
	sprintf(szSql, "SELECT site,image_dir,key_type,key_path,key_detail,new_flag FROM %s WHERE host=%d AND code_flag=%d AND used_flag=%d order by date_str desc LIMIT 16;",
			(char*)g_strTable.c_str(),ihost, CODE_FLAG, USED_FLAG );
	...
	cout<<"make key count ="<<g_lstInfo.size()<<endl;	
#if DEBUG
	printf("#%s(%d)->%s exit ! \n",__FILE__,__LINE__,__FUNCTION__);
#endif
	return ((void *)0);
...
void* thread_makeCode(void *arg)
{
	...
	cout<<"#makeCodes thread id->"<<pid<<"-& start time->"<<szTime<<endl;
	...
	while(g_lstInfo.size() > 0){		
		...
		if(tInfo.m_strSite.empty() ) {
			...
			pos = strDirName.find(tInfo.m_strSite);
			strKeyCodePath.assign(strDirName.substr(0,pos));
			strKeyCodePath.append("key");
		...
		if(fp == NULL) {
			cout<<"create key file failed:"<<codeFile<<endl;
			continue;
		}
		...
		sprintf(szSql, "UPDATE %s SET key_path='%s',code_flag=%d,update_date='%s' WHERE host=%d AND image_dir='%s' AND key_type='%s';", 
				(char*)g_strTable.c_str(), (char*)codeFile.c_str(),CODE_FLAG_UPDATE,szTime,ihost,(char*)strDirName.c_str(), (char*)tInfo.m_strKeyType.c_str() );
		mysql_query(&tMysql,szSql);
